...
from tests.unit.parametr.factories.parametr import ParametrFormData
...
class ParametrFormTest(BaseUnitTestCase):
    ...
    def test_short_name_is_optional_accepts_none(self):
        """Проверяет, что поле short_name не может быть равно None и форма не проходит валидацию."""
        form_data = ParametrFormData(
            parametr_type=self.int_type,
            par_ei=self.par_ei,
            name=self.NAME,
    ...
    def test_double_enum_parametr_par_ei_is_not_none_does_not_raise_validation_error(self):
    ...
    def test_edit_parametr_is_updated_correctly(self):
        ...
        new_form_data = ParametrFormData(
            parametr_type=self.int_type,
            par_ei=self.par_ei,
            name=self.NEW_NAME,
    ...
    def test_invalid_parametr_type_not_in_queryset_raises_error(self):
        ...
        invalid_type = ClassStructFactory(
            name="Invalid",
            short_name="Inv",
            main_class=None,
        )
