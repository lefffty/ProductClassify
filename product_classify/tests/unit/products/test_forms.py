...
from tests.unit.products.factories.product import ProdFormData, ProdFactory, ModificationFormData
from tests.unit.products.factories.par_prod import ParProdFormData
...
class ProdFormTest(BaseUnitTestCase):
    ...
    def test_name_is_required(self):
        ...
        self.assertEqual(
            form.errors["name"][0],
            "Поле для названия класса необходимо заполнить",
        )
    ...
    def test_non_jpg_or_png_image_raises_validation_error(self):
        ...
        form_files = {"image": self.GIF_IMAGE}
        form = ProdForm(data=form_data, files=form_files)
        self.assertFalse(form.is_valid())
    ...
    def test_edit_existing_object_updates_fields(self):
        ...
        new_form_data = ProdFormData(
            class_field=self.NUTS_CLASS,
        )
        new_form_files = {"image": self.JPG_IMAGE}
...
class ParProdFormTest(BaseUnitTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.nuts_class = ClassStruct.objects.get(pk=ProductsConsts.NUTS_ID)
        cls.nuts_product_class = ClassStructFactory(
        ...
        cls.INVALID_INT_PARAMETR = ParametrFactory(
            name="parameter",
            short_name="param",
            parametr_type=cls.int_parametr_cls,
        )
        ...
        # Внимание: в исходнике parclass1 перезаписывается. Переименовал в parclass3
...
class ModificationFormTest(BaseUnitTestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.empty_name_data = ModificationFormData(name="")
