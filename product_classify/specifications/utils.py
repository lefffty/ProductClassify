...
def create_total_cost_ratio_pdf(results: List[TotalCostRatioResult], product: Prod):
    font_path = os.path.join(settings.STATICFILES_DIRS[0], "fonts/DejaVuSerif.ttf")
        ...
        TableStyle(
            [
                ...
                (
                    ...
                    (0, 0),
                    (-1, 0),
                    font_name + "-Bold" if font_name == "DejaVuSans" else "Times-Bold",
                ),
            ]
        )
    )
    ...
    elements.append(Paragraph(f'СПЕЦИФИКАЦИЯ ИЗДЕЛИЯ "{product.name}"', title_style))
    ...
    signature_table_data = [[signatures[0], signatures[1], signatures[2]]]
    signature_table = Table(
        signature_table_data, colWidths=TotalCostRatioConsts.SIGNATURE_COL_WIDTHS
    )
...
def save_formset_with_logging(formset: BaseInlineFormSet, product: Prod):
    ...
    for form in formset.initial_forms:
        if form.instance.pk:
            old_objects[form.instance.pk] = ProdComponent.objects.get(
                pk=form.instance.pk
            )
